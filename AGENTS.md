...
- **Book entries** use `title, book_author, publisher, print_year, front_page (cover URL), book_url, tags, description, supp_material`. Course entries use `title, instructor, year, institute, description, personal_notes, course_url, lecture_videos, lecture_notes, supp_material`. A misspelled key (e.g. `author` instead of `book_author`) is silently ignored. URL values must be bare (no stray trailing `"`): the grids are rendered inside `_pages/resources.md`, so kramdown parses their HTML and prints any malformed tag as literal text. The includes `| escape` URLs as a safety net.
